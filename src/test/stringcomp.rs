...
fn stringcomp() {
    ...
    let keymap = test_compile_rules(keymap.context.clone(), None, None, Some("ru,ca,de,us"),
        Some(",multix,neo,intl"), None).unwrap();
    
    
    let original2 = keymap
        .get_as_string(KeymapFormat::OriginalFormat)
        .unwrap();
    ...
    assert!(Keymap::new_from_string(keymap.context.clone(), dump, KeymapFormat::TextV1, 42).is_err());
