...
pub(super) enum HandleActionError {
    ...
    #[error("The data for a private action has {max} entries; attempt to use data[{index}] ignored")]
    PrivateActionExceedMaxIndex{
        max: usize,
        index: i64
    },
...
impl From<ProcessIncludeError> for CompileKeycodesError {
    ...
    fn from(e: ProcessIncludeError) -> Self {
        ...
        match e {
            ...
            ParseFileFailed{path,error} =>
                Self::ParseKeycodesFileFailed{
                    path,error},
...
pub(crate) enum CompileSymbolsError {
    ...
    #[error("An explicit group was specified, but it provides a name for a group other than Group1")]
    NonBaseGroupName,
...
pub enum KeymapError {
    ...
    #[error("No such level: {level} for group {group}")]
    KeyNoSuchLevel {
        level: LevelIndex, group: LayoutIndex
    }
}
...
pub enum MatcherError {
    ...
    FailedToOpenXKBRules{
        rules: String, error: std::io::Error
    },
...
// TODO: move the Xkb error codes to other file
#[derive(Clone, Debug)]
