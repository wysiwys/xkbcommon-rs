...
pub fn keysym_from_name<F>(name: &str, flags: F)
    -> Option<Keysym>
    where F: TryInto<KeysymFlags>
{
        ...
        if first_char == Some('U') || first_char == Some('u') {
            ...
            if val < 0x20 || (val > 0x7e && val < 0xa0) { return None; }
            if val < 0x100 { return Some(Keysym::from(val)) ; }
            ...
            && ( second_char == Some('x') || icase && second_char == Some('X')) {
                ...
                if val > XKB_KEYSYM_MAX { return None; }
        ...
        if name.starts_with("XF86_") ||
            (icase && name[0..5].to_lowercase() == "XF86_".to_lowercase()) {

                if name.is_empty() { return None; }

                let name = String::from(&name[..4])
                    + &name[5..];
...
fn ucs_convert_case(code: u32) -> ConvertCase {
    ...
    static IPA_EXT_UPPER_MAPPING: [u16; 64] = [ /* part only */
    ...
    static LATIN_EXT_B_UPPER_MAPPING: [u16; 77] = [ /* first part only */
    ...
    static LATIN_EXT_B_LOWER_MAPPING: [u16; 77] = [ /* first part only */
...
pub fn keysym_is_keypad(sym: Keysym) -> bool {
    ...
    let sym = sym.raw();
    sym >= Keysym::KP_Space.raw()
        && sym <= Keysym::KP_Equal.raw()
...
pub fn keysym_is_modifier(sym: Keysym) -> bool {
    ...
    (sym >= Keysym::Shift_L.raw()
    && sym <= Keysym::Hyper_R.raw())
    || (sym >= Keysym::ISO_Lock.raw()
    && sym <= Keysym::ISO_Last_Group_Lock.raw())
...
fn x_convert_case(sym: Keysym) -> ConvertCase {
	...
	else if sym >= Keysym::Greek_alphaaccent && sym <= Keysym::Greek_omegaaccent &&
...
pub fn keysym_is_lower(ks: Keysym) -> bool {
    ...
    if convert.lower == convert.upper { return false; }
