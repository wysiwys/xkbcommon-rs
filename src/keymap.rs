...
impl ActionType {
    ...
    pub(crate) fn variants() -> impl Iterator<Item = ActionType> {
...
impl KeyBuilder {
    ...
    pub(crate) fn build(self) -> Result<Key, KeymapErr> {
        ...
        Ok(Key {
            ...
            out_of_range_group_action: self.out_of_range_group_action.unwrap_or_else(|| Default::default()),
            out_of_range_group_number: 
                self.out_of_range_group_number.unwrap_or_else(|| 0),
...
impl Key {
    ...
    pub(crate) fn num_levels(
        ...
        layout: LayoutIndex,
        keymap: &Keymap
    ) -> Result<LevelIndex, KeymapErr> {
        

        let layout = self.groups.get(layout)
            .ok_or(KeymapErr::KeyNoSuchGroup)?;

        // Get the corresponding type from the keymap
        let type_index = layout.key_type;
        let _type = keymap.types.get(type_index)
            .ok_or(KeymapErr::NoSuchType)?;
...
impl ModSet {
    pub(crate) fn new_from_names(
        ...
        let mods = names.into_iter()
                    .map(|name| {
                        let atom = ctx.atom_intern(
                            String::from(*name));
                        Mod {
                            name: atom,
    ...
    pub(crate) fn mod_name_to_index(
        &self,
        name: Atom,
        mod_type: ModType
    ) -> Option<ModIndex> {

        self.mods.iter().position(|_mod|
            _mod.name == name && _mod.mod_type.intersects(mod_type))
...
pub(crate) struct KeymapBuilder<T: KeymapFormatType> {
    ...
    file: Option<crate::xkbcomp::ast::XkbFile>,
    pub(crate) keycodes_section_name: Option<String>,
    pub(crate) symbols_section_name: Option<String>,
    pub(crate) types_section_name: Option<String>,
    pub(crate) compat_section_name: Option<String>,
...
impl<T: KeymapFormatType> KeymapBuilder<T> {
    ...
    fn new(
        ...
        let mods = ModSet::new_from_names(
            &builtin_mods,
            &mut context);
    ...
    pub(crate) fn build(mut self) -> Result<Keymap,KeymapErr> {

        let num_groups = self.keys.iter()
            .map(|(kc,key)| 
        ...
        keys: self.keys.into_iter().map(|(k,v)| Ok((k, v.build()?))).collect::<Result<_,KeymapErr>>()?,
...
impl Keymap {
    ...
    pub fn new_from_names<F>(
        ...
        where F:  TryInto<CompileFlags> + Into<u32> + Clone {
        ...
        let flags_u32: u32 = compile_flags.clone().into();
        let flags: CompileFlags
            = match compile_flags.try_into() {
                Ok(flags) => flags,
                Err(_) => {

                    log::error!("unrecognized flags: {:?}", flags_u32);
                    return Err(KeymapErr::UnrecognizedFlags);
    ...
    pub fn new_from_string<T,F>(
        ...
        where T: TryInto<KeymapFormat> + Into<u32> + Clone, 
    ...
    { 
        ...
        let format: KeymapFormat = match format.clone().try_into() {
            ...
            Err(_) => {
                ...
                return Err(KeymapErr::InvalidKeymapFormat)}};
    ...
    pub fn get_as_string(&self, 
        format: impl TryInto<KeymapFormat> 
        + std::marker::Copy + Into<u32>)
        -> Result<String, Box<dyn std::error::Error>> {
            ...
            let mut format: KeymapFormat = match format.try_into() {
                ...
                _ => {
                    let format: u32 = format.into();
                    log::error!("Unsupported keymap format: {}", format);
                    return Err(KeymapErr::InvalidKeymapFormat.into());
                }
            };
    ...
    where T: TryInto<KeymapFormat> {
        ...
        let format = format.try_into().map_err(|_| KeymapErr::InvalidKeymapFormat)?;

        let keymap_builder = match format {
            KeymapFormat::TextV1 => KeymapBuilder::new(context, TextV1, flags),
            _ => return Err(KeymapErr::InvalidKeymapFormat) };

        keymap_builder.keymap_new_from_file(file)
    }


    pub(crate) fn key_by_name_atom(
        &self,
        name: Atom,
        use_aliases: bool) -> Option<&Key> { todo!() }

    /// Private version of `xkb_key`
    /// that is used in State.
    pub(crate) fn xkb_key_mut<'m>(
        &'m mut self,
        kc: RawKeycode
    ) -> Option<&'m mut Key> {
    ...
    pub fn mod_get_name(&self, idx: ModIndex) -> Option<String> {
        ...
        self.context.xkb_atom_text(_mod.name)
            .map(|s| s.to_owned())
    ...
    pub fn layout_get_name(&self, idx: LayoutIndex) -> Option<String> {
        ...
        self.context.xkb_atom_text(*name)
            .map(|s| s.to_owned())
    }
    ...
    pub fn num_layouts_for_key(&self, kc: Keycode) 
        -> Option<LayoutIndex> 
    {
    ...
    pub fn num_levels_for_key(&self, kc: Keycode, layout: LayoutIndex)
    -> LevelIndex
    {
        ...
        let layout: i32 = match layout.try_into() {
            Ok(layout) => layout, _ => return 0 };

        let layout = crate::state::wrap_group_into_range(
            layout, key.groups.len(), &key.out_of_range_group_action,
            &key.out_of_range_group_number);

        let layout = match layout {
            Some(layout) => layout, None => return 0 };
    ...
    pub fn num_leds(&self) -> LedIndex { self.leds.len() }
    ...
    pub fn led_get_name(&self, idx: LedIndex) -> Option<String> {
        ...
        self.context.xkb_atom_text(led.name?)
            .map(|s| s.into())
    ...
    pub fn key_get_mods_for_level(
        ...
        if level >= key.groups.get(layout)?.levels.len() { return None; }
    ...
    pub fn key_get_syms_by_level(
        ...
        kc: Keycode,
        layout: LayoutIndex,
        level: LevelIndex) -> Result<Vec<Keysym>, KeymapErr> {

        let key = self.xkb_key(kc.raw()).ok_or(KeymapErr::NoSuchKey)?;

        let layout = crate::state::wrap_group_into_range(
            layout.try_into().map_err(|_| KeymapErr::KeyNoSuchGroup)?, key.groups.len(), 
            &key.out_of_range_group_action,
            &key.out_of_range_group_number);

        let layout = layout.ok_or(KeymapErr::KeyNoSuchGroup)?;
        

        if level >= key.num_levels(layout, self)? { 
            return Err(KeymapErr::KeyNoSuchLevel); }
        
        let level = key.groups.get(layout).ok_or(KeymapErr::KeyNoSuchGroup)?
                                .levels.get(level).ok_or(KeymapErr::KeyNoSuchLevel)?;

        // TODO: is this correct?
        let level_clone = level.syms.clone()
                            .into_iter()
                            .filter(|s| s.is_some())
                            .map(|s| s.unwrap())
                            .collect();
    ...
    pub fn min_keycode(&self) -> Keycode { Keycode::new(self.min_key_code) }
    

    pub fn max_keycode(&self) -> Keycode { Keycode::new(self.max_key_code) }

    pub fn key_get_name(&self, kc: Keycode) -> Option<String> {

        let key = self.xkb_key(kc.raw())?;

        self.context.xkb_atom_text(key.name)
            .map(|s| s.to_string())
    ...
    fn get_key_by_name(
        ...
        if let Some(key) = self.keys.values().find(|k| k.name == name) {
            return Some(key); }

        if use_aliases {
            let new_name = self.resolve_key_alias(name)?;

            return self.get_key_by_name(new_name, false);
        }
...
impl KeymapBuilder<TextV1> {
    ...
    pub(crate) fn get_key_by_name_mut(
        ...
        self.keys.values_mut().find(|k| k.name == name)
...
impl ModSet {
    ...
    // from state.c
    pub(crate) fn mod_mask_get_effective(&self, mods: ModMask)
        -> ModMask {
