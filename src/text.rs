...
use crate::keymap::{
    XKB_MOD_NONE,
    ActionControls, ActionType, ModSet,  
    MatchOperation, MOD_REAL_MASK_ALL};
use crate::rust_xkbcommon::*;


pub(crate) fn lookup_key<'s, K,V,B>(
    tab: &'static phf::OrderedMap<B,V>,
    key: &'s str) -> Option<&'static V>
    where K: Eq + Ord + phf::PhfHash 
                + std::convert::From<&'s str>,
...
use unicase::UniCase;
pub(crate) static CTRL_MASK_NAMES:
    phf::OrderedMap<UniCase<&'static str>, ActionControls>
    = phf::phf_ordered_map! {
...
pub(crate) static MOD_COMPONENT_MASK_NAMES: 
    phf::OrderedMap<UniCase<&'static str>, StateComponent>
    = phf::phf_ordered_map! {
...
pub(crate) static GROUP_COMPONENT_MASK_NAMES: 
    phf::OrderedMap<&'static str, StateComponent>
    = phf::phf_ordered_map! {
...
pub(crate) static GROUP_MASK_NAMES: 
    phf::OrderedMap<UniCase<&'static str>, u32>
    = phf::phf_ordered_map! {
...
pub(crate) static GROUP_NAMES: 
    phf::OrderedMap<UniCase<&'static str>, u8>
    = phf::phf_ordered_map! {
...
pub(crate) static USE_MOD_MAP_VALUE_NAMES: 
    phf::OrderedMap<UniCase<&'static str>, bool>
    = phf::phf_ordered_map! {
...
pub(crate) static ACTION_TYPE_NAMES: 
    phf::OrderedMap<UniCase<&'static str>, ActionType>
    = phf::phf_ordered_map! {
...
pub(crate) static SYM_INTERPRET_MATCH_MASK_NAMES: 
    phf::OrderedMap<UniCase<&'static str>, MatchOperation>
    = phf::phf_ordered_map! {
...
impl Context {
    ...
    pub(crate) fn mod_index_text<'a>(&'a self,
        mods: &ModSet, ndx: ModIndex) -> &'a str {
...
impl Context {
    ...
    pub(crate) fn si_match_text(&self, _type: &MatchOperation)
        -> String {
    ...
    pub(crate) fn mod_mask_text(
        ...
        if mask == MOD_REAL_MASK_ALL { return "all".into() }

        let text = mods.mods.iter().enumerate()
            .filter(|(i, _mod)| {
                ( mask & (1 << i) ) != 0})
            .map(|(_,_mod)| {
        ...
        for i in 0..16 {
            ...
            let s = format!("{}{}",
                match pos {
                    0 => "",
                    _ => "+" },
                lookup_value(&MOD_COMPONENT_MASK_NAMES, 1 << i, false)
                    .unwrap_or_else(|| ""));
    ...
    pub(crate) fn control_mask_text(&self, mask: ActionControls) 
        -> String {
        ...
        if mask.is_empty() { return "none".into(); }

        if mask == ActionControls::all() { return "all".into(); }
        ...
        for i in 0..16 {
            ...
            let s = format!("{}{}",
                match pos {
                    0 => "",
                    _ => "+" },
                lookup_value(&CTRL_MASK_NAMES, 1 << i, false)
                    .unwrap_or_else(|| ""));
