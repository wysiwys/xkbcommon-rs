...
impl IncludeStmtPart {

    pub(super) fn new(
        merge: MergeMode, 
        file: String,
        map: Option<String>,
        modifier: Option<String>) -> Self {
...
impl IncludeStmt {
    pub(crate) fn create(
        ...
        let maps = parse_include_maps(string, merge)
                    ...
                    .collect::<Result<Vec<IncludeStmtPart>,IncludeErr>>()?;
...
impl ExprDef {
    ...
    pub(crate) fn string(&self) -> Option<Atom> {

        if let ExprDef::String(s) = self {
            return Some(s.str);
        }
...
impl ExprIdent {
    ...
    pub(crate) fn create(ident: Atom) -> Result<ExprDef, KeymapErr> {

        let op = ExprOpType::Ident;
        let value_type = ExprValueType::Unknown;


        Ok( ExprDef::Ident( Self { op, value_type, ident } ) )
    }
...
impl ExprString {
    ...
    pub(crate) fn create(str: Atom) -> Result<ExprDef,KeymapErr> {

        let op = ExprOpType::Value;
        let value_type = ExprValueType::String; //?

        Ok( ExprDef::String( Self { op, value_type, str } ) )
...
impl ExprBoolean {
    ...
    pub(super) const fn new_false() -> Self {
        ...
        Self { op, value_type, set: false }


    }
    
    pub(crate) fn create(set: bool) -> Result<ExprDef,KeymapErr> {

        let op = ExprOpType::Value;
        let value_type = ExprValueType::Boolean;  //?

        Ok(ExprDef::Boolean(Self { op, value_type, set }))
...
impl ExprInteger {
    ...
    pub(crate) fn create(ival: i64) -> Result<ExprDef,KeymapErr> {

        let op = ExprOpType::Value;
        let value_type = ExprValueType::Int; //?

        Ok(ExprDef::Integer(Self { op, value_type, ival }))
...
impl ExprFloat {
    ...
    pub(crate) fn create(_float: f64) -> Result<ExprDef,KeymapErr> {
...
impl ExprKeyName {
    ...
    pub(crate) fn create(key_name: Atom) -> Result<ExprDef,KeymapErr> {

        let op = ExprOpType::Value;
        let value_type = ExprValueType::Keyname; //?

        Ok(ExprDef::KeyName(Self { value_type, op, key_name }))
...
impl ExprBinary {
    ...
    pub(crate) fn create(
        op: ExprOpType, 
        left: ExprDef, 
        right: ExprDef) -> Result<ExprDef,KeymapErr> {
...
impl ExprFieldRef {
    pub(crate) fn create(
        ...
        Ok(ExprDef::FieldRef(Self { op, value_type, element, field }))
...
impl ExprArrayRef {
    pub(crate) fn create(
        ...
        Ok(ExprDef::ArrayRef(Self { op, value_type, element, field, 
            entry: Box::new(entry) }))
...
impl ExprAction {
    pub(crate) fn create(
        ...
        Ok(ExprDef::Action(Self { op, value_type, name, args }))
    ...
    pub(crate) fn create(
        ...
        let actions = actions.into_iter()
            .map(|x| {
                ...
                if let ExprDef::Action(ref a) = x {
                    return Ok(Box::new(x));
                } else {
                    return Err(AstError::MustBeAction.into());
                }
            })
            .collect::<Result<Vec<Box<ExprDef>>,KeymapErr>>()?;

        Ok(ExprDef::Actions(Self { op, value_type, actions }))

    }

    pub(crate) fn push_action(
            &mut self,
            action: ExprDef) -> Result<(),KeymapErr> {

        if let ExprDef::Action(_) = action {
            let action_expr = Box::new(action);

            self.actions.push(action_expr);
            
            return Ok(());
        }

        Err(AstError::IsNotAction.into())
...
impl VarDef {
    pub(crate) fn create(name: Option<ExprDef>, value: ExprDef) -> Result<Self,KeymapErr> {
    ...
    pub(crate) fn create_bool(ident: Atom, set: bool) 
        -> Result<Self, KeymapErr> {
...
impl VModDef {
    pub(crate) fn create(name: Atom, value: Option<ExprDef>) -> Result<Self,KeymapErr> {
...
impl KeycodeDef {
    ...
    pub(crate) fn create(name: Atom, value: i64) -> Result<Self, KeymapErr> {
...
impl KeyAliasDef {
    ...
    pub(crate) fn create(alias: Atom, real: Atom) -> Result<Self,KeymapErr> {
...
impl KeyTypeDef {
    pub(crate) fn create(name: Atom, body: Vec<VarDef>) -> Result<Self,KeymapErr> {
...
impl SymbolsDef {
    pub(crate) fn create(key_name: Atom, symbols: Vec<VarDef>) -> Result<Self,KeymapErr>{

        let merge = MergeMode::default();
        
        Ok(Self { merge, key_name, symbols })
...
impl ModMapDef {
    ...
    pub(crate) fn create(modifier: Atom, keys: Vec<ExprDef>) -> Result<Self,KeymapErr> {
...
pub(crate) struct GroupCompatDef {
    pub(crate) merge: MergeMode,
    group: usize, //c type: unsigned
    def: ExprDef
}

impl GroupCompatDef {

    pub(crate) fn create(group: usize, val: ExprDef) -> Result<Self,KeymapErr> {
        let merge = MergeMode::default();

        Ok(Self { merge, group, def: val })
...
impl InterpDef {
    ...
    pub(crate) fn create(sym: Option<Keysym>, _match: Option<ExprDef>) -> Result<Self,KeymapErr> {


        let merge = MergeMode::default();

        Ok(Self { merge, sym, _match, def: vec![]} )
...
impl LedNameDef {
    pub(crate) fn create(ndx: usize, name: ExprDef, _virtual: bool)
        -> Result<Self, KeymapErr> {

        let merge = MergeMode::default();

        Ok(Self { merge, ndx, name, _virtual })
    }
...
impl LedMapDef {
    ...
    pub(crate) fn create(name: Atom, body: Vec<VarDef>) 
        -> Result<Self,KeymapErr> {
...
impl XkbFile {
    pub(crate) fn create(
        ...
        flags: XkbMapFlags) -> Result<Self,KeymapErr> {
    ...
    pub(crate) fn from_components(
        ctx: &mut Context,
        kkctgs: ComponentNames) -> Result<Self,KeymapErr> 
    {
        let components = [
        ...
        let file = XkbFile::create(XkbFileType::Keymap,
            None, Some(defs), None, 
            XkbMapFlags::empty());
    ...
    pub(crate) fn take_files(
        &mut self) -> impl Iterator<Item=Self> {
    
        self.files.take().unwrap_or(vec![]).into_iter()
    }

    pub(crate) fn file_type(&self) -> XkbFileType { self.file_type }
}
