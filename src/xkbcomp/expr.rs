            ...
            match self {
                ...
                ExprDef::ArrayRef(ar) => {
                    ...
                    let elem = match ar.element {
                        Some(elem) => Some(ctx.xkb_atom_text(elem)?)
                            .map(|s| s.into()),
                        None => None };
...
impl ExprDef {
    ...
    pub(crate) fn resolve_boolean(
        ...
        -> Option<bool> {
            ...
            match self {
                expr if expr.op_type() == Value => {
                    if expr.value_type() != ExprValueType::Boolean {
                        log::error!("{:?}:Found constant of type {:?} where boolean was expected",
            ...
            ExprDef::FieldRef(field_ref) if field_ref.op == FieldRef => { 
                ...
                log::error!("{:?}: Default \"{}.{}\" of type boolean is unknown",
            ...
            ExprDef::Unary(unary) if unary.op == Invert  => {
                return (*unary.child).resolve_boolean(
                            ctx);
                },
            ...
            _ => log::error!("{:?}: {:?} of boolean values not permitted", XkbError::InvalidOperation, self.op_type())
    ...
    pub(crate) fn resolve_keycode(
        ...
        -> Option<RawKeycode> {
            ...
            expr if expr.op_type() == Value => {
                ...
                log::error!("{:?}: Found constant of type {:?} where int was expected",
                    XkbError::WrongFieldType,
                    expr.value_type());
                return None;
            },
            ExprDef::Binary(binary) 
                if [Add, Subtract,Multiply,Divide].contains(&binary.op) => {
            ...
            expr => {
                log::error!("{:?}: Unknown operator {:?} in ResolveKeyCode",
                    XkbError::InvalidSyntax, expr.op_type());
    ...
    pub(crate) fn resolve_integer_lookup<
        T, F: FnOnce(usize, &Context) -> Option<T>>(
        self,
        lookup_fn: F,
        ctx: &Context)
        -> Option<i64> 
...

    {
            ...
            match self {
                ...
                expr if expr.op_type() == Value => {
                    ...
                    if expr.value_type() == ExprValueType::Int {
                        return expr.int();

                    } else { 
                        log::error!("{:?}: Found constant of type {:?} where an int was expected",
                            XkbError::WrongFieldType, expr.value_type());
                        return None;
                ...
                ExprDef::Ident(ident) =>  {
                        ...
                        if resolved.is_none() {
                            log::error!("{:?}: Identifier \"{}\" of type int is unknown",
                ...
                ExprDef::FieldRef(fr) => { 
                    ...
                    log::error!("{:?}: Default \"{}.{}\" of type int is unknown",
                        XkbError::InvalidExpressionType,
                ...
                ExprDef::Binary(e) if [Add, Subtract, Multiply, Divide].contains(&e.op) => {
                    ...
                    let l = (*e.left).resolve_integer_lookup(
                        lookup_fn, ctx)?;
                    let r = (*e.right).resolve_integer_lookup(
                            lookup_fn, ctx)?;
                    ...
                    match e.op {
                        ...
                        Divide => {
                            if r == 0 {
                                log::error!("{:?}: Cannot divide by zero: {} / {}",
                                    XkbError::InvalidOperation, l, r);
                                return None;
                            }
                            return Some(l / r);
                        },
                        _ => {
                        log::error!("{:?}: {:?} of integers not permitted",
                            XkbError::InvalidOperation,
                            e.op);
                ...
                expr if expr.op_type() == Assign => { 
                    ...
                    log::error!("{:?}: Assignment operator not implemented yet",
                        XkbError::InvalidOperation);
                ...
                expr if expr.op_type() == Not => { 
                    ...
                    log::error!("{:?}: The ! operator cannot be applied to an integer",
                        XkbError::InvalidOperation);
                ...
                ExprDef::Unary(unary) if [Invert, Negate].contains(&unary.op) => {
                    ...
                    let l = (*unary.child).resolve_integer_lookup(
                        lookup_fn, ctx)?;
                ...
                ExprDef::Unary(unary) if unary.op == UnaryPlus => {
                    return (*unary.child).resolve_integer_lookup(
                        lookup_fn, ctx);
                },
                expr => { 

                    log::error!("{:?}: Unknown operator {:?} in ResolveInteger",
                        XkbError::UnknownOperator, expr.op_type());
    ...
    pub(crate) fn resolve_integer(
        ...
        -> Option<i64> {
            ...
            // TODO: why None?
            self.resolve_integer_lookup::<u32,_>(
                |_, _| None, ctx)
    }
    ...
    pub(crate) fn resolve_group(
        ...
        -> Option<LayoutIndex> {
            ...
            let result =
                self.resolve_integer_lookup::<u8,_>(
                    |ident, ctx| {
            ...
            if result <= 0 || result > XKB_MAX_GROUPS.into() {
                log::error!(
                    "{:?}: Group index {} is out of range (1..{})", XkbError::UnsupportedGroupIndex,
    ...
    pub(crate) fn resolve_level(
        ...
        -> Option<LevelIndex> {
            ...
            if result < 1 {
                log::error!(
                    "{:?}: Shift level {} is out of range", XkbError::UnsupportedShiftLevel, result);
                return None;
            }
    ...
    pub(crate) fn resolve_string(
        ...
        -> Option<Atom> {
            ...
            match self {
                ...
                _ => {log::error!(
                    "{:?}: {:?} of strings not permitted", XkbError::InvalidSyntax, self.op_type());
                return None;}
    ...
    pub(crate) fn resolve_enum<T,F>(
        self,
        ctx: &Context,
        f: F)
        -> Option<T>
        where F: Copy + FnOnce(&str) -> Option<T>,
              T: std::fmt::Debug
...

    {
            ...
            if self.op_type() != ExprOpType::Ident {
                log::error!(
                    ...
                    XkbError::WrongFieldType, self.op_type());
                return None;
            }
            ...
            if let ExprDef::Ident(ident) = self {
                ...
                if let Some(value) = f(&s) {
                    ...
                    log::error!(
                        "{:?}: Illegal identifier {}", 
                        XkbError::InvalidIdentifier, ident.ident);
    ...
    fn resolve_mask_lookup<I, T, F>(
        self,
        ctx: &Context,
        lookup: F,
        )
        -> Option<T>
        where F: Copy + FnOnce(I,ExprValueType, &Context) -> Option<T>,
              I:  std::convert::TryFrom<usize> +
...

    {
            ...
            match self {
                ...
                expr if expr.op_type() == Value => {
                    ...
                    log::error!("{:?}: Found constant of type {:?} where a mask was expected",
                        XkbError::WrongFieldType, expr.value_type());
                    return None;
                ...
                ExprDef::Ident(ident) => {
                    ...
                    if value.is_none() {
                        ...
                        log::error!("{:?}: Identifier {:?} of type int is unknown", 
                            err, ctx.xkb_atom_text(ident.ident));
                        return None;
                    }
                    return value;
                ...
                ExprDef::ArrayRef(_) => {
                    log::error!("{:?}: Unexpected array reference in mask expression; Expression ignored",
                        XkbError::WrongFieldType);
                    return None;

                },
                ExprDef::Action(_) => {
                    log::error!("{:?}: Unexpected function use in mask expression; Expression ignored",
                        XkbError::WrongFieldType);
                    return None;
                },
                ExprDef::Binary(binary) if [Add, Subtract, Multiply, Divide]
                    .contains(&binary.op) => {

                        let l = (*binary.left).resolve_mask_lookup(ctx,
                            lookup)?;
                        let r = (*binary.right).resolve_mask_lookup(ctx,
                                lookup)?;
                ...
                ExprDef::Unary(unary) 
                    if [UnaryPlus, Negate, Not].contains(&unary.op) => {
                    ...
                    match v {
                        ...
                        None => {
                            ...
                            log::error!("{:?}: The {:?} operator cannot be used with a mask",
                                XkbError::InvalidOperation, 
                                match unary.op { Negate => "-", _ => "!" });
    ...
    pub(crate) fn resolve_mask<I,T,F>(
        self,
        ctx: &Context,
        f: F) -> Option<T>
        where F: Copy + FnOnce(I, ExprValueType, &Context) -> Option<T>,
    ...
    pub(crate) fn resolve_mod_mask(
        ...
        -> Option<ModMask> {
            ...
            self.resolve_mask_lookup(ctx,
                |field, _value_type, ctx| {
                    ...
                    match str.as_str() {
                        ...
                        "none" => return Some(0),
                         _ => {
                             let ndx = mods.mod_name_to_index(field,
                                 mod_type)?;
    ...
    pub(crate) fn resolve_keysym(
        ...
        -> Option<Keysym> {
            ...
            if val < XKB_KEYSYM_MIN.into() {
                log::warn!("{:?}: unrecognized keysym \"-0x{}\" ({})",
                XkbWarning::UnrecognizedKeysym, -1 * val, val);
                return None; }
            ...
            if val <= XKB_KEYSYM_MAX.into() {
                log::warn!("{:?}: numeric keysym \"0x{}\" ({})",
                XkbWarning::NumericKeysym, val, val);
    ...
    pub(crate) fn resolve_mod(
        ...
        -> Option<ModIndex> {
            ...
            let ident = match self {
                ...
                log::error!("{:?}: Cannot resolve virtual modifier:
            ...
            if ndx.is_none() {
                ...
                log::error!("{:?}: Cannot resolve virtual modifier:
                {:?} was not previously declared.", err,
