...
impl KeymapWriter {
    ...
    fn write_vmods(&mut self, ctx: &Context,
        keymap: &Keymap) 
    ...
    fn write_keycodes(&mut self, keymap: &Keymap)
    -> Result<(),Box<dyn std::error::Error>>{
    ...
    fn write_types(&mut self, keymap: &Keymap)
    -> Result<(),Box<dyn std::error::Error>>{
        ...
        for _type in keymap.types.iter() {
            ...
            write!(&mut self.buf, "\ttype \"{}\" {{\n",
                ctx.xkb_atom_text(_type.name)
                .unwrap_or_else(|| ""))?;

            write!(&mut self.buf, "\t\tmodifiers= {};\n",
                ctx.mod_mask_text(&keymap.mods,
                    _type.mods.mods))?;
            ...
            for entry in _type.entries.iter() {
                ...
                let s = ctx.mod_mask_text(&keymap.mods,
                    entry.mods.mods);
...
s, entry.level + 1)?;
                ...
                if entry.preserve.mods != 0 {
                    ...
                    write!(&mut self.buf, "\t\tpreserve[{}]= {};\n",
                        s, ctx.mod_mask_text(&keymap.mods,
                            entry.preserve.mods))?;
    ...
    fn write_led_map(&mut self,
        ctx: &Context, led: &Led, keymap_mods: &ModSet) 
    -> Result<(),Box<dyn std::error::Error>>{

        write!(&mut self.buf, "\tindicator \"{}\" {{\n",
            match led.name.as_ref() {
                Some(name) => ctx.xkb_atom_text(*name),
                None => None}.unwrap_or_else(|| "".into()))?;

        if !led.which_groups.is_empty() {

            if led.which_groups != StateComponent::LAYOUT_EFFECTIVE {

                write!(&mut self.buf, "\t\twhichGroupState= {};\n",
        ...
        if !led.which_mods.is_empty() {
            ...
            if led.which_mods != StateComponent::MODS_EFFECTIVE {
                write!(&mut self.buf, "\t\twhichModState= {};\n",
...
impl ActionFlags {
    ...
    fn affect_lock_text(&self, show_both: bool) -> String {
        ...
        match self.clone().intersection( ActionFlags::LockNoLock | ActionFlags::LockNoUnlock ) {
            e if e == ActionFlags::empty() => match show_both {
                true => ",affect=both", false => "" },
...
impl KeymapWriter {
        ...
        match action {
            Action::Mods(m) => {
                    ...
                    if action_type != ActionType::ModLock
                        && m.flags.intersects(ActionFlags::LockClear) { ",clearLocks" }
                        else { "" },
                    if action_type != ActionType::ModLock
                        && m.flags.intersects(ActionFlags::LatchToLock) { ",latchToLock" }
                        else { "" },
                    if action_type == ActionType::ModLock {
            ...
            Action::Group(g) => {
                    ...
                    write!(&mut self.buf, "{}{}(group={}{}{}{}){}", prefix, _type,
                    if !g.flags.intersects(ActionFlags::AbsoluteSwitch)
                        && group > 0 { "+" } else { "" },
                    if g.flags.intersects(ActionFlags::AbsoluteSwitch) { group + 1 } else { group },
                    if action_type != ActionType::GroupLock
                        && g.flags.intersects(ActionFlags::LockClear) { ",clearLocks" } else { "" },
                    if action_type != ActionType::GroupLock
                        && g.flags.intersects(ActionFlags::LatchToLock) { ",latchToLock" } else { "" },
                    suffix)?;
            ...
            Action::Ptr(p) if p.action_type == PtrMove => {
                ...
                write!(&mut self.buf, "{}{}(x={}{},y={}{}{}){}", prefix, _type,
                    if !p.flags.intersects(ActionFlags::AbsoluteX) && x >= 0 { "+" } else { "" },
                    x,
                    if !p.flags.intersects(ActionFlags::AbsoluteY) && y >= 0 { "+" } else { "" },
                    y,
                    if p.flags.intersects(ActionFlags::Accel) { "" } else { ",!accel" },
                    suffix
                    )?;
            ...
            Action::Dflt(d) if d.action_type == PtrDefault => {
                ...
                write!(&mut self.buf, "affect=button,button={}{}",
                    if !d.flags.intersects(ActionFlags::AbsoluteSwitch)
                        && value >= 0 { "+" } else { "" },
                    value)?;
            ...
            Action::Screen(s) if s.action_type == ActionType::SwitchVT => {
                ...
                write!(&mut self.buf, "{}{}(screen={}{},{}same){}", prefix, _type,
                if !s.flags.intersects(ActionFlags::AbsoluteSwitch) && screen >= 0 { "+" } else  { "" },
                screen,
                if s.flags.intersects(ActionFlags::SameScreen) { "" } else { "!" },
                suffix)?;
            ...
            Action::Ctrls(c) if [CtrlSet, CtrlLock].contains(&c.action_type) => {
                ...
                write!(&mut self.buf, "{}{}(controls={}{}){}", prefix, _type,
                ctx.control_mask_text(c.ctrls),
                if c.action_type == CtrlLock { c.flags.affect_lock_text(false) } else { "".into() },
                suffix)?;
            },
    ...
    fn write_compat(&mut self, keymap: &Keymap)
    -> Result<(),Box<dyn std::error::Error>>{
        ...
        for si in keymap.sym_interprets.iter() {
            ...
            if let Some(vmod) = si.virtual_mod {
                ...
                write!(&mut self.buf, "\t\tvirtualModifier= {};\n",
    ...
    fn write_keysyms(&mut self, ctx: &Context, keymap: &Keymap,
        key: &Key, group: LayoutIndex)
    -> Result<(),Box<dyn std::error::Error>>{


        let num_levels = match key.num_levels(group, keymap) {
            Ok(n) => n, Err(_) => return Err(todo!()) };

        for level in 0..num_levels {

            if level != 0 { write!(&mut self.buf, ", ")?; }
            ...
            match num_syms {
                ...
                1 => write!(&mut self.buf, "{:>15}",
                    ctx.keysym_text(&syms[0]))?,
                _ => {
    ...
    -> Result<(),Box<dyn std::error::Error>>{
        ...
        if explicit_types {
            ...
            if multi_type {
                for (idx, group) in key.groups.iter().enumerate() {
                    ...
                    if !group.explicit_type { continue; }

                    let _type = keymap.types.get(group.key_type).unwrap();
                    write!(&mut self.buf, 
                        "\n\t\ttype[Group{}]= \"{}\",",
                        idx + 1, ctx.xkb_atom_text(_type.name)
                        .unwrap_or_else(|| "".into()))?;
        ...
        if key.vmodmap != 0 && key.explicit.intersects(
            ExplicitComponents::VMODMAP) {
        ...
        let show_actions 
            = key.explicit.clone() & ExplicitComponents::INTERP;
            ...
            for (idx, group) in key.groups.iter().enumerate() {
                ...
                if !show_actions.is_empty() {
                    ...
                    let num_levels = match key.num_levels(idx, keymap) {
                        Ok(n) => n, Err(e) => return Err(e.into())};
                    for level in 0..num_levels {
                        if level != 0 { write!(&mut self.buf, ", ")?; }
    ...
    fn write_symbols(&mut self, keymap: &Keymap)
    -> Result<(),Box<dyn std::error::Error>>{
        ...
        for (idx, group) in keymap.group_names.iter().enumerate() {
            ...
            write!(&mut self.buf, "\tname[Group{}]=\"{}\";\n",
                idx + 1, ctx.xkb_atom_text(*group)
                .unwrap_or_else(|| "".into()))?;
            ...
            for (_, key) in keymap.keys.iter() {
                ...
                if (key.modmap & (1 << mod_idx)) != 0 {
                    ...
                    if !had_any {
                        ...
                        write!(&mut self.buf, "\tmodifier_map {} {{ ",
                            ctx.xkb_atom_text(_mod.name)
                            .unwrap_or_else(|| "".into()))?;
                    }
...
impl Keymap {
    ...
    pub(crate) fn text_v1_keymap_get_as_string(&self)
