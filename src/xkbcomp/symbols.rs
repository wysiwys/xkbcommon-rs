...
struct KeyInfo {
    ...
    merge: MergeMode,
    name: Atom,
    groups: BTreeMap<LayoutIndex,GroupInfo>, // guarantee iteration order?
    repeat: KeyRepeat,
    vmodmap: ModMask,
...
impl GroupInfo {
    ...
    fn copy(&mut self, from: Self) {

        self.defined = from.defined;
...
impl ModMapEntryKey {
    ...
    fn keysym_text(&self, ctx: &Context) 
        -> String {
        use ModMapEntryKey::*;
        ...
        -> Result<(),SymbolsErr>
    {
        ...
        for (i, from_level) in from.levels.iter().enumerate() {
            ...
            if self.levels.get_mut(i).is_none() {
                ...
                if from_level.action
                    .as_ref()
                    .map(|a| a.action_type())
                        .unwrap_or_else(|| ActionType::None) == ActionType::None {
                            // Empty for consistency with other comparisons

                } 
                else if into_level.action
                    .as_ref()
                    .map(|a| a.action_type())
                        .unwrap_or_else(|| ActionType::None) == ActionType::None {
        ...
        self.defined 
            |= from.defined.intersection(GroupField::ACTS);
        self.defined 
            |= from.defined.intersection(GroupField::SYMS);
...
fn use_new_key_field(
    field: KeyField, old: KeyField, new: KeyField,
    clobber: bool, collide: &mut KeyField) -> bool {
...
impl KeyInfo {
    ...
    fn merge(&mut self, ctx: &Context, 
        from: &KeyInfo, same_file: bool)
        -> Result<(), SymbolsErr> {

            let info_text = from.info_text(ctx);
            ...
            if use_new_key_field(KeyField::VMODMAP,
                self.defined, from.defined, clobber,
                &mut collide) {
            ...
            if use_new_key_field(KeyField::REPEAT,
                self.defined, from.defined, clobber,
                &mut collide) {
            ...
            if use_new_key_field(KeyField::DEFAULT_TYPE,
                self.defined, from.defined, clobber,
                &mut collide) {
            ...
            if use_new_key_field(KeyField::GROUPINFO,
                self.defined, from.defined, clobber,
                &mut collide) {

                self.out_of_range_group_action 
                    = from.out_of_range_group_action.clone();
                self.out_of_range_group_number 
                    = from.out_of_range_group_number;
            ...
            if report && !collide.is_empty() {
                ...
                log::warn!("{:?}: Symbol map for key {} redefined;
                 Using {} definition for conflicting fields", err,
                 ctx.key_name_text(self.name), 
...
impl SymbolsInfo {
    ...
    fn add_mod_map_entry(&mut self, ctx: &Context, new: ModMapEntry)
    -> Result<(), SymbolsErr> {
        ...
        for old in self.modmaps.iter_mut() {
            ...
            if new.have_symbol != old.have_symbol
                || new.u != old.u // Just compare the enum variants
            {
                    continue; 
            }

            if new.modifier == old.modifier { return Ok(()); }
    ...
    fn merge_included_symbols(&mut self, 
        builder: &KeymapBuilder<TextV1>,
        from: SymbolsInfo,
        merge: MergeMode) {
        ...
        if self.keys.is_empty() {
            ...
            for mut keyi in from.keys {
                ...
                if let Err(e) = self.add_key_symbols(
                    builder, keyi, false) {
                    self.error_count += 1;
                }
    ...
    fn handle_include_symbols(
        ...
        -> Result<(), SymbolsErr> {
            ...
            let mut included = SymbolsInfo::new(
                &mut builder.context, self.actions.clone(), self.mods.clone());


            for stmt in include.iter_stmts() {

                let file = match super::include::process_include_file(
                    &mut builder.context, stmt, XkbFileType::Symbols) {
                    Ok(file) => file,
                    Err(e) => {
                        self.error_count += 10;
                        return Err(SymbolsErr::IncludeFailed);
                    } };

                let mut next_incl = SymbolsInfo::new(
                    &mut builder.context, self.actions.clone(), included.mods.clone());
                ...
                next_incl.handle_symbols_file(
                    builder, file, MergeMode::Override);

                included.merge_included_symbols(
                    builder, next_incl, stmt.merge);
            ...
            self.merge_included_symbols(
                builder, included, first_merge);
...
impl KeyInfo {
    ...
    fn get_group_index(&mut self, ctx: &Context, array_ndx:
        Option<ExprDef>, what: LookupType) -> Option<LayoutIndex> {
        ...
        if array_ndx.is_none() {
            ...
            let mut group = match array_ndx.resolve_group(ctx) {
                Some(ndx) => ndx,
                None => {
                    todo!();
                    return None;
...
impl SymbolsInfo {
    fn add_symbols_to_key(&mut self, ctx: &Context, mut keyi: Option<&mut KeyInfo>,
        array_ndx: Option<ExprDef>, value: ExprDef)
        -> Result<(), SymbolsErr> {

            let ndx;
            if let Some(ref mut keyi) = keyi {
                ndx = match keyi.get_group_index(
                    ctx, array_ndx, LookupType::Symbols) {
                    Some(ndx) => ndx,
                    None => return Err(todo!()) };
            } else {
                ndx = match self.default_key.get_group_index(
                    ctx, array_ndx, LookupType::Symbols) {
                    Some(ndx) => ndx,
                    None => return Err(todo!()) };
            ...
            let groupi = match keyi_to_change.groups.get_mut(&ndx) {
                Some(g) => g, None => return Err(todo!()) };
            ...
            if groupi.defined.intersects(GroupField::SYMS) {
                ...
                log::error!("{:?}: Symbols for key {}, group {} already defined; Ignoring duplicate definition",
                    err, 
            ...
            for (i, syms_list) in value.syms_lists.into_iter().enumerate() {


               let mut leveli = Level {
                    action: None,
                    syms: syms_list };
    ...
    fn add_actions_to_key(&mut self,
        ctx: &Context,
        ...
        -> Result<(), SymbolsErr> {
            ...
            let ndx = match keyi.get_group_index(
                ctx, array_ndx, LookupType::Actions) {
                Some(ndx) => ndx,
                None => return Err(todo!()) };

            let groupi = keyi.groups.get_mut(&ndx)
                .ok_or(SymbolsErr::GroupNotCreated)?;
            ...
            if groupi.defined.intersects(GroupField::ACTS) {
                log::warn!("{:?}: Actions for key {}, group {} already defined",
                    XkbMessageCode::NoId,
                    keyi.info_text(ctx),
                    ndx);
            ...
            while action.is_some() {
                ...
                for level in groupi.levels.len()..n_acts {
                ...
                for i in 0..n_acts {
                    ...
                    let val = self.actions.handle_action_def(ctx, &self.mods, current_action);
                    if let Err(e) = val {
    ...
    fn set_symbols_field(&mut self, ctx: &Context, keyi: Option<&mut KeyInfo>, field: String,
        array_ndx: Option<ExprDef>, value: ExprDef) -> Result<(), SymbolsErr> {
        ...
        if field_str == "type" {
            ...
            if array_ndx.is_none() {
                ...
                if let Some(keyi) = keyi {
                    keyi.groups.insert(ndx, GroupInfo {
                        _type: Some(val),
                        defined: GroupField::TYPE,
                        levels: vec![] });
                } else {
                    self.default_key.groups.insert(ndx, GroupInfo {
                        _type: Some(val),
                        defined: GroupField::TYPE,
                        levels: vec![] });
                }
            ...
            keyi.vmodmap = match value.resolve_mod_mask(
                ctx, ModType::VIRT, &self.mods) {
                Some(mask) => mask,
                None => {
            ...
            log::warn!("{:?}: Key behaviors not supported; Ignoring locking specification for key {}",
                XkbWarning::UnsupportedSymbolsField,
                todo!());

        } else if ["radiogroup", "permanentradiogroup", "allownone"].contains(&field_str) {

            log::warn!("{:?}: Radio groups not supported; Ignoring radio group specification for key {}",
                XkbWarning::UnsupportedSymbolsField,
                todo!());

        } else if field_str.starts_with("overlay")
        || field_str.starts_with("permanentoverlay") {
            
            log::warn!("{:?}: Overlays not supported; Ignoring overlay specification for key {}",
                XkbWarning::UnsupportedSymbolsField,
                todo!());
            ...
            // TODO: check case
            let val = match value.resolve_enum(
    ...
    fn set_group_name(&mut self, ctx: &Context, 
        array_ndx: Option<ExprDef>, value: ExprDef)
        -> Result<(), SymbolsErr> {
            ...
            let group = match array_ndx.unwrap().resolve_group(ctx) {
                Some(group) => group,
                None => {
                    let err = XkbError::UnsupportedGroupIndex;
            ...
            let name = match value.resolve_string(ctx) {
                ...
                None => {
                    let err = XkbError::WrongFieldType;
                    log::error!("{:?}: Group name must be a string; Illegal name for group {} ignored", err, group);
                    return Err(todo!());
                }
            };
    ...
    -> Result<(), SymbolsErr>
    {
        ...
        let lhs = match lhs {
            Some(lhs) => lhs, None => return Err(todo!()) };
        ...
        if elem == Some("key".into()) {
            ...
            ret = self.set_symbols_field(ctx, None,
            ...
            log::error!("{:?}: Global \"groupswrap\" not supported; Ignored", XkbWarning::UnsupportedSymbolsField);
            ret = Ok(());
        } else if elem.is_none() && ["groupsclamp", "clampgroups"].contains(&field_str) {


            log::error!("{:?}: Global \"groupsclamp\" not supported; Ignored", XkbWarning::UnsupportedSymbolsField);
            ret = Ok(());
        } else if elem.is_none() && ["groupsredirect", "redirectgroups"].contains(&field_str) {


            log::error!("{:?}: Global \"groupsredirect\" not supported; Ignored", XkbWarning::UnsupportedSymbolsField);
            ret = Ok(());
        } else if elem.is_none() && field_str == "allownone" {

            log::error!("{:?}: Radio groups not supported; Ignoring \"allownone\" specification",
                XkbWarning::UnsupportedSymbolsField);
            ret = Ok(());
    ...
    fn handle_symbols_body(&mut self, ctx: &Context, defs: Vec<VarDef>, keyi: &mut KeyInfo)
        -> Result<(), SymbolsErr> {
            ...
            for def in defs {
                ...
                if let Some(ref name) = def.name {
                    ...
                    if name.op_type() == ExprOpType::FieldRef {
                        ...
                        log::error!("{:?}: Cannot set a global default value from within a key statement; Move statements to the global file scope", todo!());
                        continue;
                ...
                if ok.is_ok() {
                    ...
                    ok = self.set_symbols_field(ctx, Some(keyi), field,
                        array_ndx, def.value);
                }
    ...
    fn handle_symbols_def(&mut self, 
        builder: &KeymapBuilder<TextV1>,
        stmt: SymbolsDef)
        -> Result<(), SymbolsErr> {
            ...
            if let Err(e) = self.handle_symbols_body(
                &builder.context, stmt.symbols, &mut keyi) {
                self.error_count += 1; return Err(e) };

            if let Err(e) = self.set_explicit_group(
                &builder.context,
&mut keyi) {
                self.error_count += 1; return Err(e) };
    ...
    fn handle_mod_map_def(
        ...
        -> Result<(), SymbolsErr> {
            ...
            let modifier_name  = ctx.xkb_atom_text(def.modifier)
                .map(|n| n.to_lowercase());


            let ndx;
            if modifier_name == Some("none".into()) {

                ndx = None;

            } else {

                let mod_ndx = self.mods.mod_name_to_index(
                    def.modifier, ModType::REAL);
                if mod_ndx.is_none() {
                    todo!();
                    return Err(todo!());
                }
    ...
    fn handle_symbols_file(&mut self, 
        builder: &mut KeymapBuilder<TextV1>,
        file: XkbFile, merge: MergeMode) {
        ...
        for stmt in file.defs {
            ...
            ok = match stmt {
                Decl::Include(s) 
                    => self.handle_include_symbols(builder, s),
                Decl::Symbols(s) 
                    => self.handle_symbols_def(builder, s),
                Decl::Var(s) => self.handle_global_var(&builder.context, s),
                Decl::VMod(s) 
                    => self.mods.handle_vmod_def(
                        &builder.context, s, merge).map_err(|e| todo!()),
            ...
            if self.error_count > 10 {
                ...
                log::error!("{:?}: Abandoning symbols file \"{}\"",
...
impl KeymapBuilder<TextV1> {
    ...
    fn find_key_for_symbol_mut<'k>(&'k mut self, sym: Keysym)
        -> Option<&'k mut KeyBuilder> {
                ...
                loop {
                    ...
                    for kc in self.keys.keys() {
                        if let Some(groups) = self.keys[kc].groups.as_ref() {
                            if group < groups.len()
                                && level < groups[group].levels.len() {

                                    got_one_group = true;
                                    got_one_level = true;

                                    let syms = &groups[group]
                                        .levels[level]
                                        .syms;
                    ...
                    level += 1;
                    if !got_one_level || found_kc.is_some() { break; }
                }
                group  += 1;
                if !got_one_group || found_kc.is_some() { break; }
            }
          
        found_kc.map(|kc| self.keys.get_mut(&kc).expect("algorithm to find key for symbol failed"))
...
impl GroupInfo {
    ...
    fn find_automatic_type(&self, ctx: &mut Context)
        -> Option<Atom> {
            ...
            let sym0 = self.get_first_sym_at_level(0).unwrap_or_else(|| xkeysym::NO_SYMBOL);
            let sym1 = self.get_first_sym_at_level(1).unwrap_or_else(|| xkeysym::NO_SYMBOL);
            ...
            if width <= 4 {
                ...
                if keysym_is_lower(sym0) && keysym_is_upper(sym1) {
                    ...
                    let sym2 = self.get_first_sym_at_level(2)
                        .unwrap_or_else(|| xkeysym::NO_SYMBOL);
                    let sym3 = if width == 4 {self.get_first_sym_at_level(3) }
                        else { None };
...
impl KeyInfo {
    ...
    fn copy_symbols_def_to_keymap(
        ...
        let num_groups = self.groups.values()
            .filter(|g| !g.defined.is_empty()).count();

        if num_groups == 0 {
            return Err(SymbolsErr::NoGroups.into());
        }
        ...
        for i in group_indices.iter() {
            ...
            let (explicit_type, type_index) = builder.find_type_for_group(
                &self, *i);
            ...
            if num_levels < self.groups[i].levels.len() {
                ...
                log::warn!("{:?}: Type {:?} has {} levels, but {} has {} levels; Ignoring extra symbols",
                    XkbWarning::ExtraSymbolsIgnored,
                    builder.context.xkb_atom_text(
                        type_name)
                        .unwrap_or_else(|| "")
                        .to_owned(),
                    num_levels, self.info_text(&builder.context),
        ...
        let mut key = match key {
            ...
            None => {
                ...
                log::warn!("{:?}: Key {} not found in keycodes; Symbols ignored",
                    err,
                    info_text);
    ...
    fn copy_mod_map_def_to_keymap( 
        ...
        let key_to_edit = match self.u {
            ...
            Keysym(s) => {
                ...
                match builder.find_key_for_symbol_mut(s) {
                    Some(key) => key,
                    None => {
...
impl SymbolsInfo {
    ...
    fn copy_symbols_to_keymap(mut self, 
        builder: &mut KeymapBuilder<TextV1>)
        -> Result<(), KeymapErr> {
            ...
            for mm in self.modmaps {
                ...
                if let Err(_) = mm.copy_mod_map_def_to_keymap(builder, &self.mods) {
                    self.error_count += 1;
                }
            }
...
pub(super) fn compile_symbols(
    ...
    let mut info = SymbolsInfo::new(
        ctx, actions, builder.mods.clone());
    info.default_key.merge = merge;

    info.handle_symbols_file(builder, file, merge);

    if info.error_count != 0 {
        return Err(KeymapErr::CouldNotCompileSymbols);
    }
