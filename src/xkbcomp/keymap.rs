...
impl KeymapBuilder<TextV1> {
    ...
    // from keycodes.c
    fn compile_keycodes(
        &mut self, file: XkbFile, merge: MergeMode)
        -> Result<(),KeymapErr>
    { 
    ...
    fn compile(&mut self, file: XkbFile, merge: MergeMode) 
        -> Result<(),KeymapErr> {
    ...
    pub(crate) fn compile_keymap(
        &mut self, mut xkb_file:XkbFile, merge: MergeMode)
        -> Result<(), KeymapErr> { 
        ...
        for file in xkb_file.take_files() {
                ...
                if file_type < XkbFileType::first_type() 
                    || file_type > XkbFileType::last_type() {
        ...
        if !ok { return Err(AstError::KeymapCreationFailed.into());}

        // Compile sections
        for file_type in XkbFileType::iter_possible() {

            log::debug!("Compiling {:?}", file_type);
            let file: XkbFile = files.remove(&file_type)
...
impl ModAction {
    ...
    fn update_action_mods(
        &mut self, modmap: ModMask,
        mods: &ModSet
        ) {

        if self.flags.intersects(
            ActionFlags::ModsLookupModMap) {
            self.mods.mods = modmap; }

        self.mods.mask  = 
            mods.compute_effective_mask(
                self.mods);
...
fn find_interp_for_key(
    ...
    syms: &Vec<Keysym>,
    key: &Key,
    group: LayoutIndex, level: LevelIndex)
    -> Option<SymInterpret> {
        ...
        for interp in interps.iter() {
            ...
            if interp.sym.is_some() {
                ...
                else if interp.sym.as_ref() != syms.get(0) { continue; }
            }
...
impl Keymap {
    ...
    fn apply_interps(&mut self, kc: &RawKeycode)
        ...
        let mut vmodmap = 0;
        if self.keys.get(kc).is_none() {
            return Err(todo!()) }

        if self.keys[kc]
            .explicit.intersects(ExplicitComponents::INTERP) {
            return Ok(()) }
            ...
            for g_idx in 0..self.keys[kc].groups.len() {
                ...
                for l_idx in 0..self.keys[kc].groups[g_idx].levels.len() {
                    ...
                    let interp = find_interp_for_key(
                        &self.sym_interprets,
                        &self.key_get_syms_by_level(
                            Keycode::new(*kc), g_idx, l_idx)?,
                        &self.keys[kc], g_idx, l_idx);

                    let interp = match interp {
                        Some(interp) => interp, None => continue };
                    ...
                    if g_idx == 0 && l_idx == 0 {
                        if !self.keys[&kc].explicit.intersects(ExplicitComponents::REPEAT) && interp.repeat {
                    ...
                    if let Some(action) = interp.action {
                        ...
                        let key = self.keys.get_mut(&kc).unwrap();
                        key.groups[g_idx]
                            .levels[l_idx].action = Some(action);
        ...
        if !self.keys[&kc]
            .explicit.intersects(ExplicitComponents::VMODMAP) {
                let key = self.keys.get_mut(&kc).unwrap();
                key.vmodmap = vmodmap; }
...
impl Keymap {
    ...
    pub(super) fn update_derived_keymap_fields(&mut self) -> Result<(), KeymapErr> {
        ...
        // to reflect the vmods.
            for _type in self.types.iter_mut() {
               ...
               for entry in _type.entries.iter_mut() {
                   ...
                   entry.mods.mask 
                       = self.mods.compute_effective_mask(entry.mods);
                   entry.preserve.mask 
                       = self.mods.compute_effective_mask(entry.preserve);
